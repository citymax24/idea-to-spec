...
7. **Changelog.** Insert a new block directly under the intro paragraph of `CHANGELOG.md`, above the previous newest block: `## v<new> · <date> · from round R<n>` with a table `Change | Affects | Trigger | Type`, one row per applied item, then `Not changed: R<n>-03 (question, answered)` and `Deferred: …` lines.
