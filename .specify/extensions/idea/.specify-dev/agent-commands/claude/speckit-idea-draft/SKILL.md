...
metadata:
  ...
  source: idea:commands/speckit.idea.draft.md
---
...
7. **Git.** If the project is a git repository: `git add FEATURE_DIR`, commit `spec(<slug>): v0.1 initial draft`, tag `spec-<slug>-v0.1`. The slug is the feature directory name without its numeric prefix. If not a repository, say so and skip; the version history then exists only in `CHANGELOG.md`.
